		...
		timer.Create(uniqueID, 0.25, 0, function()
			if (IsValid(client)) then
				...
				if (client:GetMoveType() != MOVETYPE_NOCLIP and character) then
					...
					runSpeed = nut.config.get("runSpeed") + (character:getAttrib("stm", 0)/5)
					...
					if (current != value) then
						...
						if (value == 0 and !client:getNetVar("brth", false)) then
							...
							--removed because stat whores ruin everything
							--character:updateAttrib("end", 0.05)
							--character:updateAttrib("stm", 0.05)
