local PLUGIN = PLUGIN
	...
	function t_cassette.ReceiveButtonClick(_, pl)
		...
		if IsValid(ent) and ent:GetClass() == "nut_cassette_player" and ent:CanClick() then
...
function PLUGIN:LoadCassettes()
	local fields = self.getData and self:getData() or {}
	...
	for k, v in pairs(fields) do
		...
		--]]
		local itemTable = nut.item.list[v.class]
		if (itemTable and itemTable.isCassette) then
			entity:SetCassetteKey(itemTable.key)
		end
