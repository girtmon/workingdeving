...
nut.command.add("removeflesh", {
	onRun = function(client, arguments)
		local char = client:getChar()
		
		if(!hasTrait(client, "removeflesh")) then
			client:notifyLocalized("You are not afflicted with the horrible sin.")
			return false
		end
		...
		if(math.abs(tonumber(lastDust) - tonumber(os.date("%d"))) >= 2) then -- once every 2 days.
			...
			nut.item.spawn("food_humanflesh", client:getItemDropPos())
			client:notifyLocalized("You have removed the tumor from your body. Blood pulsates from the area before it quickly heals. You feel minimal pain.")
		else
			client:notifyLocalized("You can only extract your horrible tumour off of your body once every 2 days.")
		end
	end
})
