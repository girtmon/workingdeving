...
nut.chat.register("sco", {
    onCanSay =  function(speaker, text)
		return hasTrait(speaker, "sco")
	end,
	onChatAdd = function(speaker, text)
		if (hasTrait(LocalPlayer(), "sco")) then
			chat.AddText(nut.config.get("chatColor"),speaker:getChar():getName()..' gurgles, creating unintelligable sounds, "'..text..'"')
		else
			chat.AddText(nut.config.get("chatColor"),speaker:getChar():getName().." gurgles, creating unintelligable sounds.")
		end
	end,
	onCanHear = nut.config.get("chatRange", 280),
	onGetColor = function(speaker, text)
