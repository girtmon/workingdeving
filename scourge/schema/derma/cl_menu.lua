...
local PANEL = {}
	...
	function PANEL:Init()
		...
		self.background = self:Add("DImage")
		self.background:SetSize(ScrW(), ScrH())
		self.background:SetImage("background.png")
		...
		self.pp = self:Add("DPanel")
		self.pp:SetSize(ScrW(), ScrH())
		self.pp.Paint = function (self, w, h)
		...
		self.title:SetSize(self.panel:GetWide(), 65)
	...
	local bgMat = Material("materials/schema/background.png")

	function PANEL:Paint(w, h)

		nut.util.drawBlur(self, 0)
	...
	function PANEL:addTab(name, callback, uniqueID)
		...
		local function paintTab(tab, w, h)
			...
			if (self.activeTab == tab) then
				...
				draw.SimpleText(string.upper(name), "cpFontBs", w/2, h-16, Color(169, 0, 16), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			elseif (tab.Hovered) then
				surface.SetDrawColor(0, 0, 0, 50)
				surface.DrawRect(0, h - 8, w, 8)
				draw.SimpleText(string.upper(name), "cpFont", w/2, h-16, Color(255, 0, 0), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			else
				draw.SimpleText(string.upper(name), "cpFont", w/2, h-16, Color(97, 0, 1), TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM)
			end
		end

		surface.SetFont("cpFont")
