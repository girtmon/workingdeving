...
do
	...
	nut.char.registerVar("model", {
			...
			if (faction) then
				for k, v in SortedPairs(faction.models) do
					...
					icon.DoClick = function(this)
						panel.payload.model = v
						
						if(!preview) then
							preview = panel:Add("DModelPanel")
							
							
							preview:SetPos(ScrW() * 0.5 - (ScrW() * 0.35)/2, y/5 + 10)			// Set the position
							preview:SetSize(ScrW() * 0.35, ScrH()/2)		// Set the size
		...
		onValidate = function(value, data)
			...
			if (faction) then
				if(!data.model or !table.HasValue(faction.models,data.model) ) then
		...
		onAdjust = function(client, data, value, newData)
			...
			if (faction) then
				...
				local model = faction.models[data.model]
