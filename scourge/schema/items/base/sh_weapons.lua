...
ITEM:hook("drop", function(item)
	if (item:getData("equip")) then
		...
		if (IsValid(weapon)) then
			...
			item.player:EmitSound("inbs/equip.wav", 80)
		end
	end
end)
...
ITEM.functions.Equip = {
	...
	onRun = function(item)
        ...
        if type(item.minimumStats)=="table" then
            local char = client:getChar()
            for k,v in pairs(item.minimumStats) do
                if char:getAttrib(k, 0)<v then
                    client:notifyLocalized("notEnough"..k)
                    return false
                end
            end
        end
		...
		if (IsValid(weapon)) then
			...
			client:EmitSound("inbs/equip.wav", 80)
...
ITEM.functions.Annoint = {
	name = "Annoint",
	icon = "icon16/wrench.png",
	onRun = function(item)
		local client = item.player
		local dust = client:getChar():getInv():hasItem("j_scrap_holywater")
		client:requestQuery("Are you sure you want to annoint this weapon?", "Annoint", function(text)
			dust:remove()
			item:setData("customName", "Annointed " .. item:getName())
			item:setData("customDesc", item:getDesc(true) .. "\nThis weapon glows a faint yellow.")
			item:setData("customCol", Color(255, 255, 51))
	...
	onCanRun = function(item)
		local client = item.player or item:getOwner()
		return (item:getData("infused") == nil) and client:getChar():getInv():hasItem("j_scrap_holywater") and (!IsValid(item.entity))
	end
}
